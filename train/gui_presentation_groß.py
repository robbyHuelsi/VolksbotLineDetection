...
class ImgAndCmdWindow():
    def __init__(self, imgAndCmdList, subfolderList, inverseCmds=True, showInfo=False):
        ...
        style.configure("trueNeg.Horizontal.TProgressbar",
                        troughcolor='dark green', background="black", thickness=72)
        style.configure("truePos.Horizontal.TProgressbar",
                        troughcolor="black", background='dark green', thickness=73)
        style.configure("true.Label", background="black", foreground='dark green', font=("Helvetica", 65))
        ...
        self.pbTrueVelYawNeg = Progressbar(self.window, orient=HORIZONTAL,
                                           length=4.0*(self.windowWidth/9)-10, mode="determinate",
                                           style="trueNeg.Horizontal.TProgressbar")
        self.pbTrueVelYawNeg["maximum"] = 1
        self.pbTrueVelYawNeg["value"] = 0
        self.pbTrueVelYawNeg.grid(row=2, column=0, columnspan=3, sticky="S")
        ...
        self.pbTrueVelYawPos.grid(row=2, column=5, columnspan=3, sticky="S")


        self.svTrueVelYaw = StringVar(value="0 %")
        Label(self.window, textvariable=self.svTrueVelYaw, style="true.Label", width=5, anchor="center").grid(row=2, column=4, sticky="S")
...
if __name__ == "__main__":
    recordingsFolder = os.path.join(os.path.expanduser("~"),
                                    "volksbot", "data", "train_lane_curve")
